...
이벤트는 선택적으로 변수 데이터를 포함하는 서술적인 메세지로 묘사된다. (데이터는 각각의 이벤트마다
다를 가능성이 있다.) 이벤트는 개발자가 이벤트의 원인을 알 수 있는 중요성을 지니고 있다; 이
중요성은 *레벨(level)*\ 이나 *중요도(severity)*\ 라고 한다.
...
표준 레벨과 적응성(applicability)은 (중요도의 증가 순으로) 아래에 설명되어 있다:
...
이 코드를 스크립트에 쓰고 실행시키면 아래 결과를 보게 된다:
...
실제 출력은 필요하다면 꽤 융통성있게 포매팅될 수 있다;
포매팅 옵션도 추후에 설명할 것이다.
